...
describe('Header', () => {
  ...
  it('toggles menu', () => {
    render(<Header />);
    expect(screen.queryByTestId('right-section').className.includes('col-md-10')).toBeTruthy();
    fireEvent.click(screen.queryByTestId('toogle-menu-icon'));
    expect(screen.queryByTestId('right-section').className.includes('col-md-11')).toBeTruthy();
  });
});
