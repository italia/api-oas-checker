...
function listSchemas(document) {
  const schemas = {};
  if (document && document.components && document.components.schemas) {
    Object.keys(document.components.schemas).forEach((schemaName) => {
      const schema = document.components.schemas[schemaName];
      if (schema.properties) {
        schemas[schemaName] = renderSchemaText(schema);
      } else {
        console.log(`Schema ${schemaName} has no properties.`);
      }
    });
  }
  return schemas;
...
function renderExternalDocs(externalDocs) {
  if (externalDocs.description) {
    return `[${externalDocs.description}](${externalDocs.url})`;
  }
  if (externalDocs.url) {
    return `[${externalDocs.url}](${externalDocs.url})`;
  }
  return '-';
}
function renderSchemaText(schema) {
  ...
  Object.keys(schema.properties).forEach((propertyName) => {
    ...
    const required = property.required && property.required.includes(propertyName) ? 'REQUIRED' : '';
    const reference = renderExternalDocs(property.externalDocs);
    const datatype = `JSON ${property.type}`;
    const description = `**${required}** ${property.description}`;
    console.log(`${propertyName} ${datatype} ${description} ${reference}`);
    schemaProperties[propertyName] = `${propertyName} ${datatype} ${description} ${reference}`;
  });
  return schemaProperties;
}

export const SchemaDescriptorPanel = () => {
  try {
